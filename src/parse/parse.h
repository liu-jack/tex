...
public:
	...
	bool isInteger() const { return m_type == NumberType_Integer; }
	bool isFloat() const { return m_type == NumberType_Float; }
	bool isSigned() const { return m_str[0] == '-'; }
	int64_t toSignedInt() const { return (int64_t)toUnsignedInt(); }
	uint64_t toUnsignedInt() const { char *end = (char *)m_str.c_str() + m_str.size(); return strtoull(m_str.c_str(), &end, 0); }
...
public:
	...
	SdpAssignValue &value() { return m_value; }
private:
...
public:
	...
	void cleanup();
	ParseContextPtr getCurContext() { return m_contextStack.top(); }
