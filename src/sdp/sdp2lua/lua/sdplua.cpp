...
void LuaSdpStruct::visit(T &t, bool bOpt) const
{
	...
	for (int i = 1; i <= iFieldNum; ++i)
	{
		...
		//lua_pop(L, 1);
		int iFieldNameStack = formalStackIndex(L, -1);
		poper.push(1);

		// define
		lua_pushvalue(L, iFieldNameStack);
		//lua_getfield(L, iStructDefinitionStack, sName.c_str());
		lua_gettable(L, iStructDefinitionStack);
		...
		lua_gettable(L, m_iStructStack);
		int iFieldCurValueStack = formalStackIndex(L, -1);
		poper.push(1);

		// check field
		if (lua_isnil(L, -1)) throw std::runtime_error(string("field not exist: ") + lua_tostring(L, iFieldNameStack));

		DEBUG((IsWrite ? "write" : "read") << " struct field, tag: " << iTag << ", required: " << (bRequired ? "true" : "false") << ", name: " << lua_tostring(L, iFieldNameStack));//sName);
		...
		if (IsWrite)
		{
			...
			ReaderWriterSelector<IsWrite, true>()(writer).visit(t, iTag, bRequired, lua_tostring(L, iFieldNameStack));//sName.c_str());
		}
		else
		{
			LuaSdpValueReader reader(L, iFieldCurValueStack, iFieldTypeStack);
			ReaderWriterSelector<IsWrite, false>()(reader).visit(t, iTag, bRequired, lua_tostring(L, iFieldNameStack));//sName.c_str());
			if (reader.hasNewValue())
			{
				lua_setfield(L, m_iStructStack, lua_tostring(L, iFieldNameStack));//sName.c_str());
			}
		}
	}
}
