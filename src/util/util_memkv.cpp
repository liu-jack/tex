...
public:
	...
	bool checkVersion(uint32_t iVersion) { return iVersion == 0 || iVersion == getVersion(); }
	uint32_t getVersion() { return m_pDataHead->iVersion; }
	void setVersion(uint32_t iVersion) { m_pDataHead->iVersion = iVersion; }
	uint32_t getSyncTime() { return m_pDataHead->iSyncTime; }
	void setSyncTime(uint32_t iSyncTime) { m_pDataHead->iSyncTime = iSyncTime; }
	uint32_t getAccessTime() { return m_pDataHead->iAccessTime; }
	void setAccessTime(uint32_t iAccessTime) { m_pDataHead->iAccessTime = iAccessTime; }

	bool isDirty() const { return (m_pDataHead->iDataFlags & CMemKVDataFlag_Dirty) != 0; }
	void setDirty() { m_pDataHead->iDataFlags |= CMemKVDataFlag_Dirty; }
	void clearDirty() { m_pDataHead->iDataFlags &= ~CMemKVDataFlag_Dirty; }

	bool isDeleted() const { return (m_pDataHead->iDataFlags & CMemKVDataFlag_Deleted) != 0; }
	void setDeleted() { m_pDataHead->iDataFlags |= CMemKVDataFlag_Deleted; }
