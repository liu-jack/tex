...
public:
	...
	MfwLogType getLogType() const { return m_type; }
...
protected:
	const string &getLogFilePathPrefix() const { return m_sLogFilePahtPrefix; }
...
string CLogManager::escapeLogData(const string &sData, uint32_t iMaxSize)
{
	...
	for (unsigned i = 0; i < sData.size(); ++i)
	{
		...
		switch (c)
		{
		case '\\': sResult.append("\\\\"); break;
		case '\r': sResult.append("\\r"); break;
		case '\n': sResult.append("\\n"); break;
		default:
			{
				if (c == '|')
