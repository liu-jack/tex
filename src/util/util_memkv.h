...
private:
	struct NodeInfo
	{
		...
		bool hasSyncTask() const { return iSyncTime != 0; }
		bool hasAccessLRU() const { return !bIsDirty && iAccessTime != 0; }
	};
...
public:     
    ...
    iterator begin() { return Iterator(m_mKeyNode.begin()); }
    iterator end() { return Iterator(m_mKeyNode.end()); }

private:
	bool load();
	uint64_t getNewUniqueVersion();
	bool updateKeyNode(const string &sKey, uint32_t iChunk);
	char *getChunkAddr(uint32_t iChunk) { return m_pMemBase + iChunk * m_iChunkSize; }
