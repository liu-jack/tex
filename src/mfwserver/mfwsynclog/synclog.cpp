...
string getSqlFieldType(uint32_t iFieldDataType)
{
    ...
    case FieldDataType_String:
        return "VARCHAR(256)";
    case FieldDataType_DateTime:
        return "DATETIME";
    case FieldDataType_Int32:
        return "INT";
    case FieldDataType_UInt32:
        return "INT UNSIGNED";
    case FieldDataType_Int64:
        return "BIGINT";
    case FieldDataType_UInt64:
...
string LogFieldConfig::getInsertValues(CMysql &mysql, const string &sValue) const
{
    ...
    if (!sColumn.empty()) {
        ...
        switch (emFieldDataType) {
        case FieldDataType_String:
            os << "\"" << mysql.escape(sValue) << "\"";
            break;
        case FieldDataType_DateTime:
            os << (!sValue.empty() ? ("\"" + sValue + "\"") : "NULL");
            break;
        case FieldDataType_Int32:
            os << UtilString::strto<int32_t>(sValue);
            break;
        case FieldDataType_UInt32:
            os << UtilString::strto<uint32_t>(sValue);
            break;
        case FieldDataType_Int64:
            os << UtilString::strto<int64_t>(sValue);
            break;
        case FieldDataType_UInt64:
            os << UtilString::strto<uint64_t>(sValue);
            break;
        case FieldDataType_Char:
            os << "\"" << mysql.escape(sValue) << "\"";
            break;
        case FieldDataType_Float:
            os << UtilString::strto<float>(sValue);
            break;
        }
