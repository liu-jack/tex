...
public:
	...
	void setBindAdapter(BindAdapter *pBindAdapter) { m_pBindAdapter = pBindAdapter; }
...
public:
	...
	NetServer *getNetServer() { return m_pNetServer; }

	void setEndpoint(const string &sEndpoint) { m_listenEndpoint.parse(sEndpoint); }
	const CEndpoint &getEndpoint() const { return m_listenEndpoint; }
	void setMaxConns(int iMaxConns) { m_iMaxConns = iMaxConns; }
	uint32_t getMaxConns() const { return m_iMaxConns; }
	void setQueueCapacity(uint32_t iCapacity) { m_iQueueCapacity = iCapacity; }
	uint32_t getQueueCapacity() const { return m_iQueueCapacity; }
	void setQueueTimeout(uint32_t iTimeout) { m_iQueueTimeout = iTimeout; }
	uint32_t getQueueTimeout() const { return m_iQueueTimeout; }
	void setProtocolName(const string &sName) { m_sProtocolName = sName; m_bIsMfwProtocol = m_sProtocolName == "mfw"; }
	const string &getProtocolName() { return m_sProtocolName; }
	bool isMfwProtocol() { return m_bIsMfwProtocol; }
	void setProtocol(protocol_functor fn) { m_fnProtocol = fn; }
	protocol_functor getProtocol() { return m_fnProtocol; }
	CSocket &getSocket() { return m_servsock; }
	...
	bool isLimitMaxConnection() const { return m_iCurConns + 1 > m_iMaxConns; }
	uint32_t getNowConnection() const { return m_iCurConns; }
	void decreaseNowConnection() { --m_iCurConns; }
	void increaseNowConnection() { ++m_iCurConns; }
	...
	vector<HandlePtr> &getHandles() { return m_vHandles; }
...
public:
	...
	bool isUdp() const { return m_bIsUdp; }
	void setUid(uint32_t uid) { m_iUid = uid; }
	...
	uint16_t getBindPort() const { return m_iBindPort; }
	uint32_t getTimeout() const { return m_iTimeoutMS; }
	int getfd() const { return m_clisock.getfd(); }
...
public:
	...
	map<int, BindAdapterPtr> &getAllBindAdapter() { return m_mBindAdapter; }
	void initEpoll();
	void closeConnection(uint32_t uid);
	void sendResponse(uint32_t uid, const string &sData, const string &sIp, uint16_t iPort);
	void sendResponse(uint32_t uid, const string &sData);

	Connection *getConnection(uint32_t uid) { return m_connMgr.get(uid); }
...
public:
	...
	NetThread *getNetThread() { return m_netThread; }
