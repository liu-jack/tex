...
public:
    ...
    template <typename T> T get(uint32_t iIndex)
    {
        return UtilString::strto<T>(get(iIndex));
    }
    template <typename T> T get(const string &sField)
    {
        return UtilString::strto<T>(get(sField));
    }
