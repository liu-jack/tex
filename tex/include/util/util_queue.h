...
class CTimeQueue
{
	...
	struct CTimeQueueIterator
	{
		...
		reference operator*() const { return _it->pairKeyTime; }
		pointer operator->() const { return &_it->pairKeyTime; }
		_Self &operator++() { ++_it; return *this; }
		_Self operator++(int) { _Self tmp = *this; ++_it; return tmp; }
		_Self &operator--() { --_it; return *this; }
		_Self operator--(int) { _Self tmp = *this; --_it; return tmp; }
		bool operator==(const _Self &x) { return _it == x._it; }
		bool operator!=(const _Self &x) { return _it != x._it; }
	};
...
public:
	...
	const_iterator begin() const { return const_iterator(m_setTimeQueue.begin()); }
	const_iterator end() const { return const_iterator(m_setTimeQueue.end()); }
	bool empty() const { return m_setTimeQueue.empty(); }
	uint32_t size() const { return m_setTimeQueue.size(); }
