...
{
public:
    explicit CLockGuard(T &lock) : m_lock(lock), m_bOwned(true)
    {
        m_lock.lock();
    }
    ~CLockGuard()
...
public:
    ...
    ~CThread();
    void setIdentifier(const string &sId)
    {
        m_sIdentifier = sId;
    }
    void setRoutine(tr1::function<void(CThread &)> routine)
    {
        m_routine = routine;
    }
    void setCleanUp(tr1::function<void(CThread &)> cleanup)
    {
        m_cleanup = cleanup;
    }
    void setIntervalRoutine(tr1::function<void(CThread &)> routine, uint32_t iInterval)
    {
        m_routine = routine;
        m_iInterval = iInterval;
    }
    void setTerminate(tr1::function<void()> terminate)
    {
...
public:
    ...
    ~CThreadPool();
    void setThreadNum(uint32_t iThreadNum)
    {
