...
public:
    ...
    explicit SdpUnpacker(const string &data) : m_data(NULL), m_size(0), m_pos(0)
    {
        reset(data.c_str(), data.size());
    }
    explicit SdpUnpacker(const vector<char> &vData) : m_data(NULL), m_size(0), m_pos(0)
    {
        reset(&vData[0], vData.size());
    }
    SdpUnpacker(const void *data, uint32_t size)
    {
        reset(data, size);
    }
    void reset()
